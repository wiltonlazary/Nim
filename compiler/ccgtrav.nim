...
proc genTraverseProc(c: var TTraversalClosure, accessor: PNode, typ: PType)
...
proc genTraverseProc(c: var TTraversalClosure, accessor, n: PNode) =
  ...
  of nkRecCase:
    ...
    lineF(p, cpsStmts, "switch ($1.$2) {$n", [accessor, disc.cg])
...
proc parentObj(accessor: PNode; m: BModule): PNode {.inline.} =
  if not m.compileToCpp:
    result = newDotExpr(accessor, atom"Sup")
  else:
    result = accessor
...
proc genTraverseProc(c: var TTraversalClosure, accessor: PNode, typ: PType) =
  ...
  of tyTuple:
    for i in countup(0, sonsLen(typ) - 1):
      genTraverseProc(c, rfmt(nil, "$1.Field$2", accessor, i.atom), typ.sons[i])
...
proc genTraverseProcSeq(c: var TTraversalClosure, accessor: PNode, typ: PType) =
  var p = c.p
  assert typ.kind == tySequence
  var i: TLoc
  getTemp(p, getSysType(tyInt), i)
  lineF(p, cpsStmts, "for ($1 = 0; $1 < $2->$3; $1++) {$n",
      [i.r, accessor, atom(if c.p.module.compileToCpp: "len" else: "Sup.len")])
  genTraverseProc(c, "$1->data[$2]" % [accessor, i.r], typ.sons[0])
  lineF(p, cpsStmts, "}$n", [])

proc genTraverseProc(m: BModule, origTyp: PType; sig: SigHash;
                     reason: TTypeInfoReason): PNode =
  ...
  if typ.kind == tySequence:
    genTraverseProcSeq(c, "a".atom, typ)
  else:
    if skipTypes(typ.sons[0], typedescInst).kind == tyArray:
      # C's arrays are broken beyond repair:
      genTraverseProc(c, "a".atom, typ.sons[0])
    else:
      genTraverseProc(c, "(*a)".atom, typ.sons[0])
...
proc genTraverseProcForGlobal(m: BModule, s: PSym): PNode =
